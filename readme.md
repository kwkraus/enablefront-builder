...
- **Series & session management** — Create webinar series containing multiple sessions; manage titles, schedules, and session details.
...
| Graph integration | Microsoft Graph v1 — delegated OBO flow for profile photo (`/api/v1/me/photo`) |
...
- **Delegated-only Graph permissions** — authenticated user context is required; Graph access is scoped to the OBO flow for profile photos (`/api/v1/me/photo`) only.
